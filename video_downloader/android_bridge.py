...
def open_folder(export_folder_uri: str | None = None) -> bool:
    ...
    if export_folder_uri:
        ...
        try:
            ...
            # Not every document provider supports the conversion helpers in
            # exactly the same way. The original tree Uri remains a useful
            # compatibility attempt below.
            traceback.print_exc()
        ...
        # A custom destination is configured. Opening some unrelated generic
        # Downloads location would be worse than reporting failure.
        return False
