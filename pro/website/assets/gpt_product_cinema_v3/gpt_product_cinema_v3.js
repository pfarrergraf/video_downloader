...
const sources = [...root.querySelectorAll('[data-pc3-source]')];
const sequence = [
...
function clearTimers(){timers.forEach(clearTimeout);timers=[];running=false;root.classList.remove('is-running')}
function show(id){
  const idx=Math.max(0,sequence.findIndex(s=>s.id===id));
  stepIndex=idx;
  root.dataset.pc3Phase=id;
  views.forEach(v=>v.classList.toggle('is-active',v.dataset.pc3View===id));
  number.textContent=sequence[idx].n;
  caption.textContent=sequence[idx].copy;
  progress.style.width=`${((idx+1)/sequence.length)*100}%`;
}
function play(){
  clearTimers();
  root.classList.add('is-running');
  running=true;
  if(reduced){show('success');return}
  sequence.forEach(s=>timers.push(setTimeout(()=>show(s.id),s.at)));
  timers.push(setTimeout(()=>{running=false;root.classList.remove('is-running')},10400));
}
function nextStep(){clearTimers();stepIndex=(stepIndex+1)%sequence.length;show(sequence[stepIndex].id)}
root.addEventListener('click',e=>{
  const next=e.target.closest('[data-pc3-next]');
  if(next){clearTimers();show(next.dataset.pc3Next);return}
  if(e.target.closest('[data-pc3-replay]')){play();return}
  if(e.target.closest('[data-pc3-stepthrough]')){nextStep();return}
});
motionButton.addEventListener('click',()=>{
  reduced=!reduced;
  motionButton.setAttribute('aria-pressed',String(reduced));
  motionButton.setAttribute('aria-label',reduced?'Volle Bewegung aktivieren':'Bewegung reduzieren');
...
if(!reducedBySystem){
  stage.addEventListener('pointermove',e=>{
    ...
    rig.style.setProperty('--pc3-pointer-x',`${x*10}deg`);
    rig.style.setProperty('--pc3-pointer-y',`${-y*8}deg`);
...
function initStream(){
  ...
  function arrowPoints(count){
    const pts=[]; const cx=w*.5,cy=h*.49,s=Math.min(w,h)*.155;
  ...
  let arrow=[];
  function resize(){
    const r=canvas.getBoundingClientRect();w=Math.max(1,r.width);h=Math.max(1,r.height);dpr=Math.min(devicePixelRatio||1,2);
    canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);
    const count=Math.min(240,Math.max(120,Math.round(w/4.2)));arrow=arrowPoints(count);particles.length=0;
    for(let i=0;i<count;i++){
      const group=i%3;
  ...
  function target(p,i,time){
    ...
    if(phase==='stream') return arrow[i];
    if(phase==='inside') return {x:center.x,y:center.y};
  ...
  function drawIcon(p){
    ctx.save();ctx.translate(p.x,p.y);ctx.strokeStyle=palette[p.c];ctx.fillStyle=palette[p.c];ctx.lineWidth=1.2;
  ...
  function frame(time){
    const dt=Math.min(32,time-last);last=time;ctx.clearRect(0,0,w,h);
    const phase=root.dataset.pc3Phase;
    particles.forEach((p,i)=>{
      const t=target(p,i,time);const strength=phase==='stream'?.015:phase==='inside'?.027:.0045;
      p.vx+=(t.x-p.x)*strength*(dt/16.67);p.vy+=(t.y-p.y)*strength*(dt/16.67);
      const dx=pointer.x-p.x,dy=pointer.y-p.y,dist=Math.hypot(dx,dy);
      if(dist<105&&phase!=='inside'){const f=(105-dist)/105;p.vx-=dx/Math.max(dist,1)*f*.1;p.vy-=dy/Math.max(dist,1)*f*.1}
      p.vx*=.89;p.vy*=.89;p.x+=p.vx;p.y+=p.vy;
      p.trail.push({x:p.x,y:p.y});if(p.trail.length>6)p.trail.shift();
      ctx.beginPath();p.trail.forEach((q,j)=>{if(j===0)ctx.moveTo(q.x,q.y);else ctx.lineTo(q.x,q.y)});ctx.strokeStyle=palette[p.c]+Math.round(22+(p.trail.length*4)).toString(16).padStart(2,'0');ctx.lineWidth=.7;ctx.stroke();
      ctx.globalAlpha=phase==='inside'?Math.max(0,.75-Math.hypot(p.x-w*.5,p.y-h*.49)/150):.7;
      drawIcon(p);ctx.globalAlpha=1;
    });
