...
# Include package data for the main package and explicitly include the nested
# Gothic web UI assets (static/i18n/*.json, icon.svg, manifest.webmanifest).
tmp_ret = collect_all('video_downloader')
datas += tmp_ret[0]; binaries += tmp_ret[1]; hiddenimports += tmp_ret[2]
datas += collect_data_files('video_downloader.web', includes=['static/**/*'])
